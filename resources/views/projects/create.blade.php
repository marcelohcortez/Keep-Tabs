            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-4 bg-blue-100 rounded-md">
                    ...
                    <div class="form-group mb-4">
                        ...
                        {{Form::select('client_id', $clients, null, ['class' => 'form-control', 'placeholder' => 'Define a client', 'value' => '{{ old("client_id") \}\}'])}}
                    </div>
                    <div class="form-group mb-4">
                        <p class="text-general-dark"><strong>{{Form::label('project_name', 'Project Name')}}</strong></p>
                        {{Form::text('project_name', '', ['class' => 'form-control', 'placeholder' => 'Insert project name', 'value' => '{{ old("project_name") \}\}'])}}
                    </div>
                    <div class="form-group mb-4">
                        <p class="text-general-dark"><strong>{{Form::label('project_description', 'Project Description')}}</strong></p>
                        {{Form::textarea('project_description', '', ['class' => 'form-control, project_description', 'placeholder' => 'Insert project description', 'id' => 'project_description', 'name' => 'project_description', 'value' => '{{ old("project_description") \}\}'])}}
                    </div>
                    <div class="form-group mb-4">
                        <p class="text-general-dark"><strong>{{Form::label('total_value', 'Total project value')}}</strong></p>
                        {{Form::text('total_value', '', ['class' => 'form-control', 'pattern' => '^\d*(\.\d{0,2})?$','placeholder' => 'Insert total project value', 'value' => '{{ old("total_value") \}\}'])}}
                    </div>
                    <div class="form-group mb-4">
                        <p class="text-general-dark"><strong>{{Form::label('paid_value', 'Paid value')}}</strong></p>
                        {{Form::text('paid_value', '', ['class' => 'form-control', 'pattern' => '^\d*(\.\d{0,2})?$','placeholder' => 'Insert paid value', 'value' => '{{ old("paid_value") \}\}'])}}
                    </div>
                    <div class="form-group mb-4">
